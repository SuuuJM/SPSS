...
def regnet_test(slice_volume_batch, slice_label_batch, reg_unet, reg_stn_label, pred_shape, num_slice, lbl_idx):
    ...
    for i in range(pred_shape[0]):
        ...
        for idx in range(lbl_idx_i - 1, start_i - 1, -1):
            ...
            pred_lbl_slc = reg_postprocess(pred_label[0, 0, ...].cpu().detach().numpy(), lbl_idx_i - idx,
                                           post_morph=True)
        ...
        for idx in range(lbl_idx_i + 1, end_i):
            ...
            pred_flow = reg_unet(input_moving, input_fixed)
            pred_label = reg_stn_label(input_label, pred_flow)
            pred_lbl_slc = reg_postprocess(pred_label[0, 0, ...].cpu().detach().numpy(), idx - lbl_idx_i,
                                           post_morph=True)
            reg_pred[i, ..., idx - start_i] = pred_lbl_slc
            input_label = torch.from_numpy(pred_lbl_slc[np.newaxis, np.newaxis, ...]).cuda().float()
    return reg_pred
...
def save_images(volume_batch, label_batch, label_full_batch, reg_pred, seg_pred, prediction, snapshot_path, iter_num,
                mask=None):
    nib.save(nib.Nifti1Image(volume_batch[0, 0, ...].cpu().data.numpy().astype(np.float32), np.eye(4)),
             snapshot_path + '/saveimg' + '/img_' + str(iter_num) + '.nii.gz')
    nib.save(nib.Nifti1Image(label_full_batch[0].cpu().detach().numpy().astype(np.float32), np.eye(4)),
             snapshot_path + '/saveimg' + '/gt_full_' + str(iter_num) + '.nii.gz')
    nib.save(nib.Nifti1Image(label_batch[0].cpu().detach().numpy().astype(np.float32), np.eye(4)),
             snapshot_path + '/saveimg' + '/gt_' + str(iter_num) + '.nii.gz')
    nib.save(nib.Nifti1Image(reg_pred[0].astype(np.float32), np.eye(4)),
             snapshot_path + '/saveimg' + '/reg_pred_' + str(iter_num) + '.nii.gz')
    nib.save(nib.Nifti1Image(seg_pred[0].astype(np.float32), np.eye(4)),
             snapshot_path + '/saveimg' + '/seg_pred_' + str(iter_num) + '.nii.gz')
    nib.save(nib.Nifti1Image(prediction[0].astype(np.float32), np.eye(4)),
             snapshot_path + '/saveimg' + '/pred_' + str(iter_num) + '.nii.gz')
