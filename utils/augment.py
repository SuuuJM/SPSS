...
def deform(img, displacements=None, rotates=None, zooms=None):
    ...
    for i in range(n):
        ...
        if len(zooms) < n :
            ...
            zoom = zooms[i]
        imgnewtmp = deform_grid(imgtmp, torch.Tensor(displacement), order=3, mode='nearest', rotate=rotate, zoom=zoom, axis=(2, 3))
        imgnew.append(imgnewtmp)
    return torch.stack(imgnew, 4), displacements, rotates, zooms 
